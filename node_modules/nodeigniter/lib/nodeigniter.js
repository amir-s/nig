...
var nodeigniter = function() {
	...
	this.load_sockets = function() {
		var app_sockets = self.config('app_dir')+'sockets';
		var base_socket = require(self.config('sys_dir') + 'core/socket');
		
		if (fs.existsSync(self.config('app_dir')+'core/socket.js')) {
			base_socket = extend({}, base_socket, require(self.config('app_dir')+'core/socket.js'));
		}
		fs.readdir(app_sockets, function (err, files) {
			files.forEach (function (file) {
				if (!/~$/.test(file)) {
					var name = file.split('.')[0];
					self.sockets[name] = extend({}, base_socket, require(app_sockets + '/' + file));
					self.sockets[name].setIO(self.io.of('/'+name));
					self.sockets[name].init();
				}
			});
		});
	}
